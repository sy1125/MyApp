...
[React Navigation 관련문서](https://reactnavigation.org/docs/typescript)
```shell
...
- Pressable, Button, TouchableHighlight, TouchableOpacity, TouchableWithoutFeedback, TouchableNativeFeedback(Pressable, TouchableWithoutFeedback이 ios / android 모두 호환되므로 주로 사용됨)
- navigation.navigate로 이동 가능
- navigation.push로 쌓기 가능
- navigation.goBack으로 이전으로 이동
- params 추가 가능(params에 user같은 객체를 통째로 넣지 말기, id만 넣고 나머지 복잡한 user는 글로벌 스토어에 넣기)
